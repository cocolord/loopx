...
def _prompt(request: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "Use the task packet context contract exactly. For spawn_agent, pass native_arguments.fork_context explicitly: fresh means false, does not inherit the parent conversation, and uses the task packet as its work context; forked_snapshot means true and is available only when the signed context policy allows it.",
